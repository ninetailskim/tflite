...
void RunInference(Settings* s) {
  ...
  for(int d=0;d < *output_detections; d++){
    const std::string cls=labels[int(output_classes[d])];
    const float score=output_scores[d];
    const int ymin=output_locations[4*d]*image_width;
    const int xmin=output_locations[4*d+1]*image_height;
    const int ymax=output_locations[4*d+2]*image_width;
    const int xmax=output_locations[4*d+3]*image_height;

    if(score > s->threshold){
      std::cout <<"detected "<<d<<" of "<<cls<<" with scope "<<score
...
void display_usage() {
  LOG(INFO) << "label_image\n"
            ...
            << "--threshold, -d: threshold for score\n"
            << "\n";
}
...
int Main(int argc, char** argv) {
  ...
  while (1) {
    static struct option long_options[] = {
        ...
        {"threshold", required_argument, nullptr, 'd'},
        {nullptr, 0, nullptr, 0}};
